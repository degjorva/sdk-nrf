...
static size_t dom2sz = sizeof(dom2);

int hash_all_inputs(char const *inputs[], size_t *inputs_lengths[], size_t inputs_count,
...
{
	char const *hash_array[] = {(char *)message};
...
static int ed25519_hash_privkey(char *workmem, const uint8_t *priv_key)
{
	...
	char const *hash_array[] = {(char *)priv_key};
			       ...
			       bool prehash)
{
	...
	char const *hash_array[] = {dom2, workmem + SX_ED25519_SZ, message};
	size_t *hash_array_lengths[] = {&dom2sz, &point_r_sz, &message_length};
	size_t input_count = prehash ? 3 : 2;
	size_t offset = prehash ? 0 : 1;

	return hash_all_inputs(&hash_array[offset], &hash_array_lengths[offset], input_count,
			       &sxhashalg_sha2_512, workmem + 3 * SX_ED25519_SZ);
}
...
{
	size_t point_r_sz = SX_ED25519_SZ;
	char const *hash_array[] = {dom2, point_r, workmem + SX_ED25519_SZ, message};
	size_t *hash_array_lengths[] = {&dom2sz, &point_r_sz, &point_r_sz, &message_length};
	size_t input_count = prehash ? 4 : 3;
	size_t offset = prehash ? 0 : 1;

	return hash_all_inputs(&hash_array[offset], &hash_array_lengths[offset], input_count,
			       &sxhashalg_sha2_512, workmem + SX_ED25519_SZ);
}
				   ...
				   size_t message_length, const char *signature, bool prehash)
{
	...
	char const *hash_array[] = {dom2, (char *)signature, (char *)pub_key, (char *)message};
