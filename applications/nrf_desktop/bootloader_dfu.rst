...
.. note::
   If your board target uses the Key Management Unit (KMU) feature (:kconfig:option:`CONFIG_CRACEN_KMU`), the ``kmu_push_area`` DTS node from the SoC devicetree reserves the push area for you, so your custom memory layout must leave the start of the physical RAM untouched.
   ...
   For an example of a custom memory layout that does so, see the :file:`nrf/applications/nrf_desktop/configuration/nrf54lm20dk_nrf54lm20a_cpuapp/memory_map_ram_load.dtsi` file.
