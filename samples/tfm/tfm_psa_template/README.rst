...
The sample requires that the device is provisioned with the PSA root-of-trust security parameters.
In the |NCS|, you can use the :ref:`provisioning image <provisioning_image>` sample during the development process.
...
* On nRF54L Series devices, ``west flash --recover`` also provisions the NSIB Ed25519 public key to the :ref:`CRACEN KMU <ug_kmu_guides_cracen_overview>` using the ``nrfutil device x-provision-keys`` command.
  Both samples default to the sample key in :file:`samples/tfm/common/keys/`, so no CLI argument is required for the keys to match.
  For example, if you are using the nRF54L15 DK, you can run the following commands:
...
To use your own NSIB signing key, complete the following steps:

1. Build the provisioning image sample with the ``NSIB_KEY_FILE`` :ref:`CMake option <cmake_options>` that points to the file containing the NSIB signing key.
   For example, if you are using the nRF54L15 DK, you can run the following command with the correct ``path/to/nsib_priv.pem``:
...
#. Set the same key for the PSA template sample by setting the :kconfig:option:`SB_CONFIG_SECURE_BOOT_SIGNING_KEY_FILE` Kconfig option to the same key as the one provided in the pem file in the previous step.
...
#. Rebuild the sample with the updated keys before proceeding with the firmware update as described in the next sections.
   ...
   On nRF54L, the same :file:`nsib_priv.pem` must also be passed to the provisioning image build so that its public key is provisioned to the CRACEN KMU.
   See :ref:`tfm_psa_template_provisioning_image_setup` or :ref:`provisioning_image` for the corresponding ``-DNSIB_KEY_FILE=...`` and ``west flash --recover`` commands.
